...
- Nouvelle carte, même motif que « LIEU DE RENCONTRE » / « CCN DE BRANCHE » (`padding: 14, background: "#F6F5F2", borderRadius: 8`), insérée **juste avant** la carte « EFFECTIF ENTREPRISE », rendue si `c.company` : titre « ACTIVITÉ DE L'ENTREPRISE » ; texte `c.activite || "Activité non renseignée"` (14 px ; gris `#94A3B8` si non renseignée) ; **une seule ligne de codes**, 11,5 px `#6E6A64`, rendue si `c.nafCode` : « NAF 2025 <nafCode> » + (« — <nafLibelle> » seulement si `c.nafLibelle` non vide et différent de `c.activite`) + (« · ancien code <nafRev2Code> » si `c.nafRev2Code`) — jamais un ancien code seul ; si `!c.nafCode`, ligne 11,5 px : `c.effectifSiren ? "« Actualiser » l'effectif récupère le code NAF." : "« Rechercher l'effectif » la préremplit."`. Aucune pastille, aucun `title`. (Amendé après revue : la version initiale masquait la ligne quand le libellé était identique et affichait parfois « ancien code » seul.)
...
- AE1. Fiche « Société X » (activité vide) : « Rechercher l'effectif » → la liste des candidats affiche « Activité : … » sous chaque entreprise → choisir → message « Activité préremplie : « … » » → la carte ACTIVITÉ DE L'ENTREPRISE affiche le libellé et, dessous, « NAF 2025 <code> · ancien code <code> » (sans répéter le libellé, identique) ; dans « Modifier », le champ contient le libellé et la ligne « NAF 2025 … (SIRENE, <date>) · ancien code … ».
- AE2. Même fiche : remplacer l'activité par « Études cliniques (CRO) », puis « Actualiser » : l'activité saisie est conservée, la carte affiche « Études cliniques (CRO) » et, dessous, « NAF 2025 <code> — <libellé officiel> · ancien code <code> » ; « Reprendre le libellé officiel » remet le libellé. (Le message « Activité conservée (votre saisie). » s'affiche lors du choix d'un candidat par « Rechercher l'effectif », pas sur « Actualiser » : choix assumé après revue, la carte suffit.) Amendements après revue : fiche cible relue au moment de la réponse SIRENE (`contactsRef`) ; libellé automatique retiré quand la nouvelle entreprise n'a pas de libellé ; compte du lot limité aux activités réellement écrites ; import par égalité stricte sur « Activité » ; runner chargeant les scripts `data/` référencés par le document.
