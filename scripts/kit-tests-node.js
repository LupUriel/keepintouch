...
function lireOuVide(chemin) { try { return fs.readFileSync(chemin, "utf8"); } catch (e) { return ""; } }
var docs = lireOuVide(path.join(__dirname, "..", "LISEZMOI.txt")) + "\n" + lireOuVide(path.join(__dirname, "..", "docs", "RECETTE.md"));
var scripts = [];
...
try {
  var dataScripts = [];
  var dataRe = /<script src="\.\/(data\/[^"]+)"/g;
  while ((match = dataRe.exec(html))) dataScripts.push(match[1]);
  if (!dataScripts.length) throw new Error("aucun script data/ référencé");
  dataScripts.forEach(function (chemin) { vm.runInContext(fs.readFileSync(path.join(__dirname, "..", chemin), "utf8"), sandbox, { filename: chemin }); });
